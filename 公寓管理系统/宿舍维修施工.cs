...
namespace 公寓管理系统
{
    public partial class 宿舍维修施工 : Form
    {
        private Panel panel;
        ...
        public 宿舍维修施工(Panel panel, DataGridView dataGridView,string con)
        {
            InitializeComponent();
            this.panel = panel;
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            int p2 = dataGridView1.RowCount - 1;
        ...
        private void 宿舍维修施工_FormClosed(object sender, FormClosedEventArgs e)
        {
            panel.Visible = true;
        }

        private void 宿舍维修施工_Load(object sender, EventArgs e)
        {
            CenterToParent();
            ...
            dataGridView1.Size = new System.Drawing.Size(933, 300);
            ...
            try
            {
                ...
                string sql3 = String.Format("select * from DormitoryService order by 报修单号");
                ...
                while (reader3.Read())
                {
                    ...
                    this.dataGridView2.Rows[i].Cells[0].Value = String.Format("{0}", reader3[0]);
                    this.dataGridView2.Rows[i].Cells[1].Value = String.Format("{0}", reader3[3]);
                    this.dataGridView2.Rows[i].Cells[2].Value = String.Format("{0}", reader3[4]);
                    this.dataGridView2.Rows[i].Cells[3].Value = String.Format("{0}", reader3[6]);
                }
                reader3.Close();
            }
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string s5 = dateTimePicker1.Text;
                ...
                string sqls = String.Format("select 报修单号 from DormitoryService where 报修单号='{0}'", s2);
                SqlCommand command1 = new SqlCommand(sqls, connec);
                s9 = Convert.ToString(command1.ExecuteScalar());
                ...
                string sqlss = String.Format("select * from DormitoryMaintain where 维修单号='{0}'", s1);
                SqlCommand commandss = new SqlCommand(sqlss, connec);
                SqlDataReader readerss = commandss.ExecuteReader();
                if (readerss.Read())
                {
                    MessageBox.Show("维修单号不能重复！");
                    textBox1.Clear();
                    textBox2.Clear();
                    textBox3.Clear();
                    textBox4.Clear();
                ...
                else if (s9 == " " || (!s9.Equals(s2)))
                {
                    MessageBox.Show("请重新核对报修单号！");
                    textBox1.Focus();
                    textBox1.Clear();
                }
                else
                {
                    readerss.Close();
                    string sql = String.Format("insert into DormitoryMaintain values('{0}','{1}','{2}','{3}','{4}','{5}','{6}','{7}')", s1, s2, s3, s4, s5, s6, s7, s8);
        ...
        private void button4_Click(object sender, EventArgs e)
        {
            ...
            if (num != "" && num2 == "" )
            {
                string sql = String.Format("select * from DormitoryMaintain where 维修单号='{0}'", num);
                SqlCommand command = new SqlCommand(sql, connec);
                connec.Open();
            ...
            {
                string sql = String.Format("select * from DormitoryMaintain where 维修单号='{0}' and 报修单号='{1}'", num, num2);
                SqlCommand command = new SqlCommand(sql, connec);
                connec.Open();
                SqlDataReader reader = command.ExecuteReader();
                while (reader.Read())
            ...
            {
                MessageBox.Show("请输入(维修单号)或(维修单号+报修单号)");
                textBox1.Focus();
            }
        }
        ...
        private void button5_Click(object sender, EventArgs e)
        {
            ...
            string k1 = textBox7.Text;
            string k2 = textBox1.Text;
            string k3 = textBox2.Text;
            string k4 = textBox3.Text;
            string k5 = dateTimePicker1.Text;
            ...
            if (!k1.Equals(m1)) { m[0] = textBox7.Text; } else m[0] = " ";
            if (!k2.Equals(m2)) { m[1] = textBox1.Text; } else m[1] = " ";
            if (!k3.Equals(m3)) { m[2] = textBox2.Text; } else m[2] = " ";
            if (!k4.Equals(m4)) { m[3] = textBox3.Text; } else m[3] = " ";
            if (!k5.Equals(m5)) { m[4] = dateTimePicker1.Text; } else m[4] = " ";
            ...
            if (textBox1.Text == "" || textBox2.Text == "" || textBox3.Text == "" || textBox4.Text == "" || textBox5.Text == "" || textBox7.Text == "" || textBox7.Text == "")
            {
            ...
            else
            {
                for (int w = 0; w < m.Length; w++)
                {
                    if (m[w] != " ")
                    {
                        if (w == 0)
                        {
                            string sql = String.Format("update DormitoryMaintain set 维修单号='{0}' where 维修单号='{1}'", k1, m1);
                            SqlCommand command = new SqlCommand(sql, connec);
                        ...
                        {
                            string sql = String.Format("update DormitoryMaintain set 报修单号='{0}' where 维修单号='{1}'", k2, m1);
                            SqlCommand command = new SqlCommand(sql, connec);
                            command.ExecuteNonQuery();
                        }
                        else if (w == 2)
                        {
                            string sql = String.Format("update DormitoryMaintain set 楼号='{0}' where 维修单号='{1}'", k3, m1);
                            SqlCommand command = new SqlCommand(sql, connec);
                            command.ExecuteNonQuery();
                        }
                        else if (w == 3)
                        {
                            string sql = String.Format("update DormitoryService set 寝室号='{0}' where 维修单号='{1}'", k4, m1);
                            SqlCommand command = new SqlCommand(sql, connec);
                            command.ExecuteNonQuery();
                        }
                        else if (w == 4)
                        {
                            MessageBox.Show(m1);
                            string sql = String.Format("update DormitoryService set 维修日期='{0}' where 维修单号='{1}'", k5, m1);
                            SqlCommand command = new SqlCommand(sql, connec);
                            command.ExecuteNonQuery();
                        }
                        else if (w == 5)
                        {
                            string sql = String.Format("update DormitoryMaintain set 维修人员='{0}' where 维修单号='{1}'", k6, m1);
                            SqlCommand command = new SqlCommand(sql, connec);
                            command.ExecuteNonQuery();
                        }
                        else if (w == 6)
                        {
                            ...
                            string sql = String.Format("update DormitoryMaintain set 报修原因='{0}' where 维修单号='{1}'", k7, m1);
                            SqlCommand command = new SqlCommand(sql, connec);
                            command.ExecuteNonQuery();
                        }
                        else if (w == 7)
                        {
                            string sql = String.Format("update DormitoryMaintain set 备注='{0}' where 维修单号='{1}'", k8, m1);
                            SqlCommand command = new SqlCommand(sql, connec);
                            command.ExecuteNonQuery();
                        }
                    }
        ...
        private void button6_Click(object sender, EventArgs e)
        {
            ...
            SqlConnection connec = new SqlConnection(connn);
            connec.Open();
            if (textBox1.Text == "" || textBox2.Text == "" || textBox1.Text == "" || textBox3.Text == "" || textBox4.Text == "" || textBox5.Text == "" || textBox7.Text == "" || textBox7.Text == "")
            {
            ...
            {
                string sql = String.Format("delete from DormitoryMaintain where 维修单号='{0}'", m1);
                SqlCommand command = new SqlCommand(sql, connec);
