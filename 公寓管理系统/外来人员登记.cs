...
namespace 公寓管理系统
{
    public partial class 外来人员登记 : Form
    {
        private Panel panel;
        ...
        public 外来人员登记(Panel panel, DataGridView dataGridView,string con)
        {
            InitializeComponent();
            this.panel = panel;
        ...
        private void 外来人员登记_Load(object sender, EventArgs e)
        {
            ...
            dataGridView1.Size = new System.Drawing.Size(1024, 300);
        ...
        private void 外来人员登记_FormClosed(object sender, FormClosedEventArgs e)
        {
            panel.Visible = true;
        }
        ...
        private void button5_Click(object sender, EventArgs e)
        {
            ...
            string t5 = textBox4.Text;
            string t6 = textBox5.Text;
            string t7 = textBox6.Text;
            string t8 = textBox7.Text;
            string t9 = textBox8.Text;
            ...
            connec.Open();
                ...
                string sql = String.Format("delete from ExternalPeople where 证件号码='{0}'", m4);
            ...
            if (t1==""||t2==""||t3==""||t4==""||t5==""||t6==""||t7==""||t8==""||t9=="")
            {
              MessageBox.Show("请选择对应的信息！");
                ...
                string sql2 = String.Format("insert into ExternalPeople values('{0}','{1}','{2}','{3}','{4}','{5}','{6}','{7}','{8}')", t1,t2,t3,t4,t5,t6,t7,t8,t9);
                SqlCommand command2 = new SqlCommand(sql2, connec);
                ...
                string sql3 = String.Format("select * from ExternalPeople");
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    string sql = String.Format("insert into ExternalPeople values('{0}','{1}','{2}','{3}','{4}','{5}','{6}',"
                        +"'{7}','{8}')", s1, s2, s3, s4, s5, s6, s7, s8, s9);
                    SqlCommand command = new SqlCommand(sql, connec);
        ...
        private void button6_Click(object sender, EventArgs e)
        {
            ...
            {
                string sql = String.Format("select * from ExternalPeople where 姓名='{0}' and 证件类型='{1}' and 证件号码='{2}'", name,certificate,num);
                SqlCommand command = new SqlCommand(sql, connec);
                connec.Open();
                ...
                while (reader.Read())
                {
                    ...
                    m9 = String.Format("{0}", reader[8]);
                     ...
                     textBox8.Text= String.Format("{0}", reader[8]);
        ...
        private void button7_Click(object sender, EventArgs e)
        {
            ...
            if (!k9.Equals(m9)) { m[8] = textBox8.Text; } else m[8] = " ";
            string connn = con;
            SqlConnection connec = new SqlConnection(connn);
            connec.Open();
            ...
            else
            {
                for (int w = 0; w < m.Length; w++)
                {
                    if (m[w] != " ")
                    {
                        if (w == 0)
                        {
                            string sql = String.Format("update ExternalPeople set 姓名='{0}' where 证件号码='{1}'", k1, m4);
                            SqlCommand command = new SqlCommand(sql, connec);
                            command.ExecuteNonQuery();
                        }
                        else if (w == 1)
                        {
                            string sql = String.Format("update ExternalPeople set 性别='{0}' where 证件号码='{1}'", k2, m4);
                            SqlCommand command = new SqlCommand(sql, connec);
                            command.ExecuteNonQuery();
                        }
                        else if (w == 2)
                        {
                            string sql = String.Format("update ExternalPeople set 证件类型='{0}' where 证件号码='{1}'", k3, m4);
                            SqlCommand command = new SqlCommand(sql, connec);
                            command.ExecuteNonQuery();
                        }
                        else if (w == 3)
                        {
                            string sql = String.Format("update ExternalPeople set 证件号码='{0}' where 证件号码='{1}'", k4, m4);
                            SqlCommand command = new SqlCommand(sql, connec);
                            command.ExecuteNonQuery();
                            m4 = k4;
                        }
                        ...
                        {
                            string sql = String.Format("update ExternalPeople set 楼号='{0}' where 证件号码='{1}'", k5, m4);
                            SqlCommand command = new SqlCommand(sql, connec);
                            command.ExecuteNonQuery();
                        }
                        else if (w == 5)
                        {
                            string sql = String.Format("update ExternalPeople set 寝室号='{0}' where 证件号码='{1}'", k6, m4);
                            SqlCommand command = new SqlCommand(sql, connec);
                            command.ExecuteNonQuery();
                        }
                        else if (w == 6)
                        {
                            string sql = String.Format("update ExternalPeople set 进入时间='{0}' where 证件号码='{1}'", k7, m4);
                            SqlCommand command = new SqlCommand(sql, connec);
                            command.ExecuteNonQuery();
                        }
                        else if (w == 7)
                        {
                            string sql = String.Format("update ExternalPeople set 离开时间='{0}' where 证件号码='{1}'", k8, m4);
                            SqlCommand command = new SqlCommand(sql, connec);
                            command.ExecuteNonQuery();
                        }
                        else if (w == 8)
                        {
                            string sql = String.Format("update ExternalPeople set 备注='{0}' where 证件号码='{1}'", k9, m4);
                            SqlCommand command = new SqlCommand(sql, connec);
                            command.ExecuteNonQuery();
                        }
                    }
